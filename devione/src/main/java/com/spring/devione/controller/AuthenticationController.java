...
public class AuthenticationController {
	...
	@Autowired
	public AuthenticationController(AuthenticationManager authenticationManavger, TokenUtils tokenUtils,
	...
	public ResponseEntity<?> login(@RequestBody LoginRequest loginRequest) {
		...
		if (!val.isEmpty()) {
			return ResponseEntity.badRequest()
					.body(Errors.NewHTTPCustomError(Errors.BadRequest, val, HttpStatus.BAD_REQUEST.value()));
		}
		...
		return ResponseEntity.ok(new TokenResponse(jwt, userDetails.getId(), userDetails.getName(),
				userDetails.getLastname(), userDetails.getEmail(), userDetails.getUsername(), userDetails.isActive()));
	}
	...
	public ResponseEntity<?> register(@RequestBody SignUpRequest signUpRequest) {
		...
		if (authService.existsByEmail(signUpRequest.getEmail())) {
			return ResponseEntity.badRequest()
					.body(Errors.NewHttpError(Errors.EmailAlreadyExists, HttpStatus.NOT_FOUND.value()));
		}

		if (authService.existsByUsername(signUpRequest.getUsername())) {
			return ResponseEntity.badRequest()
					.body(Errors.NewHttpError(Errors.UsernameAlreadyExists, HttpStatus.NOT_FOUND.value()));
		}

		User user = new User(signUpRequest.getName(), signUpRequest.getLastname(), signUpRequest.getEmail(),
				signUpRequest.getUsername(), passwordEncoder.encode(signUpRequest.getPassword()), true);
